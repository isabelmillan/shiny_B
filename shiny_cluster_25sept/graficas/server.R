...
# Imagen bancomer   
  output$default <- renderUI({
      tags$img(src='imagen_bancomer.jpg', heigth=150, width=150)
  })
...
# Se llama a la función de filtrado en utiles.R
  ...
  filedata<- reactive({
    ...
    filas<- nrow(filedata2())
    aleatorio<- filedata2()[sample(filas, input$muestra), ]
...
############################################################################################################
  ...
  ##### Inputs de puntos y líneas
      ...
      if (input$Points=="Points"&input$colorin == 'Ninguno')
      ...
      if (input$colorin != 'Ninguno')
        p <- p + aes_string(color=(input$colorin)) +scale_color_gradientn(colours = rainbow(5))
      #if (input$fillin != 'Ninguno')
      #  p <- p + aes_string(fill=(input$fillin)) + scale_color_gradientn(colours = rainbow(5))
